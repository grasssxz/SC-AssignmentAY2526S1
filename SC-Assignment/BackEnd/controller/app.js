...
app.post("/order", verifyToken, (req, res) => {
  ...
  orderDB.addOrder(req.userid, cart, total, (err, results) => {
    ...
    else res.status(201).json({ orderid: results.insertId });
  });
});
    ...
    (err, results) => {
      ...
      //No error, response with productid
        ...
        if (results.affectedRows < 1)
          res
            .status(500)
            .json({ message: "Nothing was updated! Product might not exist" });
        else res.status(201).json({ affectedRows: results.affectedRows });
      }
    }
  );
});
  ...
  userDB.updateUser(
    ...
    (err, results) => {
      if (err) {
        ...
        //Otherwise unknown error
        else res.status(500).json({ result: "Internal Error" });
      } else {
        console.log(results);
        if (results.affectedRows < 1)
  ...
  categoryDB.addNewCategory(category, description, (err, results) => {
    if (err) {
      ...
      if (err.code == "ER_DUP_ENTRY")
        res
          .status(422)
          .json({ message: `The category name provided already exists` });
      //Otherwise unknown error
      else res.status(500).json({ result: "Internal Error" });
    }
...
app.post("/product", verifyToken, (req, res) => {
  ...
  productDB.addNewProduct(
    ...
    (err, results) => {
      if (err) res.status(500).json({ result: "Internal Error" });
      //No error, response with productid
      else res.status(201).json({ productid: results.insertId });
    }
  );
});
  ...
  discountDB.addNewDiscount(
    ...
    (err, results) => {
      if (err) {
        if (
          err.errno == 1292 &&
          err.sqlMessage.startsWith("Incorrect datetime")
        )
          res.status(400).json({ message: "Invalid Time" });
        else res.status(500).json({ message: "Internal Error" });
      } else res.status(201).json({ discountid: results.insertId });
    }
  );
});
  ...
  function (req, res) {
    ...
    if (req.file == undefined) {
      ...
      const imageExtension =
        imageType.split("/")[imageType.split("/").length - 1];
      ...
      // Add image
        ...
        productImagesDB.addImage(
          productid,
          imageName,
          imageType,
          imagePath.slice(7, imagePath.length),
          (err, results) => {
            if (err) {
              ...
              res.status(500).json({ result: "Internal Error" });
            } else res.status(200).json({ affectedRows: results.affectedRows });
          }
        );
    }
  }
...
app.get("/product/cheapest/:categoryid", (req, res) => {
  productDB.get3CheapestFromCategory(req.params.categoryid, (err, results) => {
    if (err) res.status(500).json({ result: "Internal Error" });
      ...
      else res.status(200).json({ message: "No product" });
    }
  });
});
